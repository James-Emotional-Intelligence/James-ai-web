...
export const LoginPage: React.FC = () => {
  ...
  const fromPath = (location.state as any)?.from?.pathname || '/today';
  ...
  const onSubmit = async (data: LoginFormData) => {
    ...
    try {
      ...
      setErrorMessage(err.message || 'Đăng nhập không thành công. Vui lòng kiểm tra lại email hoặc mật khẩu.');
    } finally {
  ...
  const handleDemoLogin = async () => {
    ...
    try {
      ...
      setErrorMessage(err.message || 'Không thể đăng nhập bằng tài khoản Demo.');
    } finally {
  ...
  return (
    <div className="min-h-screen bg-[#050806] text-[#F3FAF5] flex flex-col md:flex-row">
      {/* Left Panel: Showcase */}
        ...
        <div className="w-full max-w-md space-y-6">
          ...
          <div className="p-3.5 rounded-2xl bg-[#101A13] border border-[rgba(34,197,94,0.25)] space-y-2">
            ...
            <p className="text-[11px] text-[#A9B8AE]">
              Tài khoản mẫu: <strong>minh.hocsinh@jami.edu.vn</strong> (Mật khẩu: <code className="bg-[#050806] px-1 py-0.5 rounded text-[#86EFAC]">Demo1234!</code>)
            </p>
            <button
              type="button"
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            {/* Email Field */}
            <div className="space-y-1">
              <label className="text-xs font-bold text-[#F3FAF5]">Email học sinh</label>
            ...
            <div className="space-y-1">
              <div className="flex items-center justify-between">
                <label className="text-xs font-bold text-[#F3FAF5]">Mật khẩu</label>
                <Link
                  to="/forgot-password"
