...
apiRouter.get("/dashboard/overview", requireAuth, asyncHandler(async (req, res) => {
  ...
  const currentDayDate = /* @__PURE__ */ new Date(`${todayDateStr}T00:00:00`);
    ...
    if (b.recurrenceRule) {
      ...
      if (rule.includes("FREQ=WEEKLY")) {
        if (rule.includes("INTERVAL=2")) {
          ...
          const diffWeeks = Math.floor(diffMs / (7 * 864e5));
          if (diffWeeks % 2 !== 0) return false;
        }
        const jsDayMap = { 1: "MO", 2: "TU", 3: "WE", 4: "TH", 5: "FR", 6: "SA", 7: "SU" };
